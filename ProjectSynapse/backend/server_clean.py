...
app = FastAPI(title="Project Synapse AI Agent Interface")
...
async def get_evolution_stats():
    ...
    total_score = sum(a['performance_score'] for a in evolution_agents)
    average_score = total_score / total_agents if total_agents > 0 else 0
...
async def submit_feedback(data: dict):
    ...
    for agent in evolution_agents:
        if agent['agent_id'] == agent_id:
            ...
            agent['performance_score'] = (current_score + score) / 2
            agent['feedback_count'] += 1
            break

    return {
        'success': True,
        'message': f'Feedback submitted for agent {agent_id}'
    }
